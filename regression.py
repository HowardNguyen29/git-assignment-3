...
def fit_regression_model(X, y):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if math.isclose(loss.item(), previos_loss, rel_tol=1e-5): # Change this condition to stop the training when the loss is not changing much.
            print(f"Training stopped at epoch {epoch} because the loss is not changing much.")
            break
